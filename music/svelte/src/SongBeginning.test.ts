...
import {bundesHymneSong, scales, SongBeginning, songStart, usaSong, vogelSong} from "$lib/MusicData";
...
describe('song beginning', () => {
            ...
            scales.forEach((sc) => {
                ["treble", "bass"].forEach((c) => {
                    ...
                    if ([...new Set(keys)].length != keys.length) {
                        console.log(song.getSongDescription())
                    }
                })
            })
        });
    ...
    it('knows correct stave notes', () => {
        ...
        expect(keysg).toStrictEqual(['A/4', 'B/4', 'D/5']);
